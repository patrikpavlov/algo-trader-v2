...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("book_writer.log"), logging.StreamHandler()]
)
...
class DatabaseWriter:
    ...
    def handle_snapshot_message(self, snapshot_data: Dict[str, Any]) -> Optional[tuple]:
        try:
             return ( snapshot_data['time'], 'binance', snapshot_data['symbol'], snapshot_data['bids'], snapshot_data['asks'] )
    ...
    async def write_batch_to_db(self, stream_name: str, data: List[tuple]) -> bool:
        ...
        sql_map = {
            "snapshots_stream": "INSERT INTO orderbook_snapshots (time, exchange, symbol, bids, asks) VALUES (to_timestamp($1), $2, $3, $4, $5) ON CONFLICT (time, exchange, symbol) DO NOTHING"
        }
    ...
    async def run(self):
        ...
        await self.process_stream("snapshots_stream", "snapshots_group", "book-writer-1", self.handle_snapshot_message)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logger.info("Book writer service stopped.")